...
class KD(nn.Module):
    ...
    def forward(self, teacher_logits: torch.Tensor, student_logits: torch.Tensor, target: torch.Tensor, epoch) -> torch.Tensor:
        if self.mode == 'normal':
            ...
            k = max(1, int(num_samples * percent))
            _, lowest_entropy_indices = torch.topk(entropy, k, largest=False)
            _, highest_entropy_indices = torch.topk(entropy, k, largest=True)
            
            mask = torch.ones(num_samples, dtype=torch.bool, device=entropy.device)
            mask[lowest_entropy_indices] = False
            mask[highest_entropy_indices] = False
            flipped_mask = ~mask
            ...
            with torch.no_grad():
                ...
                mask1 = torch.ones(num_samples, dtype=torch.bool, device=entropy.device)
                mask1[lowest_entropy_indices] = False
                
                mask2 = torch.ones(num_samples, dtype=torch.bool, device=entropy.device)
                mask2[highest_entropy_indices] = False
                flipped_mask1 = ~mask1
                flipped_mask2 = ~mask2
                
                mask = torch.logical_and(mask1, mask2)
            
            new_student_logits = student_logits * mask.unsqueeze(-1) / self.T + student_logits * flipped_mask1.unsqueeze(-1) / (self.T + ADD_CONST) + student_logits * flipped_mask2.unsqueeze(-1) / (self.T - ADD_CONST)
            p_s = F.log_softmax(new_student_logits, dim=-1)
            
            new_teacher_logits = teacher_logits * mask.unsqueeze(-1) / self.T + teacher_logits * flipped_mask1.unsqueeze(-1) / (self.T + ADD_CONST) + teacher_logits * flipped_mask2.unsqueeze(-1) / (self.T - ADD_CONST)
            p_t = F.softmax(new_teacher_logits, dim=-1)
            
            kl_loss = F.kl_div(p_s, p_t, reduction='none').sum(1)
            kl_loss = kl_loss * mask * (self.T**2) + kl_loss * flipped_mask1 * ((self.T+ADD_CONST)**2) + kl_loss * flipped_mask2 * ((self.T-ADD_CONST)**2)
            ce_loss = self.ce_criterion(student_logits, target)
